...
import type { GenerationJob } from "@kdp/db";
import {
  gridMysteryInputSchema,
  sectionLayoutSchema,
  toEngineDifficulty,
  type GridMysteryInput,
  type PageImageAssignments,
  type SectionLayout,
} from "@kdp/shared";
import {
  applyOverrides,
...
async function resolvePageImages(
  userId: string,
  assignments: PageImageAssignments | undefined,
): Promise<Record<string, Uint8Array>> {
  const entries = Object.entries(assignments ?? {}).filter((entry): entry is [string, string] =>
    Boolean(entry[1]),
  );
 ...
 */
async function loadCustomPages(userId: string): Promise<RenderCustomPage[]> {
...
/** The puzzles this book should print, plus any per-puzzle title/subtitle the publisher typed. */
interface BookPuzzles {
 ...
 */
async function loadBookPuzzles(bookId: string, input: GridMysteryInput): Promise<BookPuzzles> {
  ...
  if (rows.length === 0) {
    ...
    const puzzles = Array.from({ length: input.puzzleCount }, () =>
      generateGridMystery({
  ...
  for (const row of rows) {
    ...
    const puzzle = generateGridMystery({
      ...
      difficulty: toEngineDifficulty(row.difficulty as GridMysteryInput["difficulty"]),
      themeId: input.theme,
      seed: row.seed,
    });
...
function asPuzzleOverrides(value: unknown): PuzzleOverrides | null {
  return value && typeof value === "object" && !Array.isArray(value)
    ? (value as PuzzleOverrides)
    : null;
}
 ...
 */
export async function processJob(job: GenerationJob): Promise<void> {
  ...
  try {
    ...
    const { pdf, pageCount } = await renderGridMysteryPdf(puzzles, {
      ...
      floorStrength:
        (input.interiorColor === "blackAndWhite"
